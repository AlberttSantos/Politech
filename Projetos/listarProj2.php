...
$cont = 0;	
	$resultado = mysql_query("select * from projeto where STATUS = 'A';");
	...
	if(mysql_num_rows($resultado)<=0)
	{
		...
		echo "<b>Atenção!</b><br>";
		echo "Não existe projetos cadastrados no momento.";
		echo "</div>";
	}
	...
	else
	{
		...
		while ($linha = mysql_fetch_array($resultado))
		{
			...
			echo "<td>$linha[23]</td>";
			if ($nivel >= 4) {
			echo "<td><a href='editarProj.php?cdProj={$linha[0]}'><i class='icon-edit'></i></a></td>";
			echo "<td><a href='#' class='confirm-delete' data-id='{$linha[0]}'><i class='icon-trash'></i></a></td>";
			}
			echo "<td><a href='visualizarProj.php?cdProj={$linha[0]}'><i class='icon-search'></i></a></td>";
...
<div id="modal-from-dom" class="modal hide fade">
    ...
    <div class="modal-footer">
        <a href="deletarProj.php?cdProj=" class="btn danger">Sim</a>
