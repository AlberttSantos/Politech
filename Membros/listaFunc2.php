...
$cont = 0;	
	$resultado = mysql_query("select * from funcionario f, cargo c where f.cdcargo = c.cdcargo and f.STATUS = 'A';");
	...
	if(mysql_num_rows($resultado)<=0)
	{
		...
		echo "<b>Atenção!</b><br>";
		echo "Não existe membros cadastrados no momento.";
		echo "</div>";
	}
	...
	else
	{
		...
		while ($linha = mysql_fetch_array($resultado))
		{
			...
			echo "<td>$linha[12]</td>";
			if ($nivel >= 4) {
			echo "<td><a href='editarFunc.php?cdFunc={$linha[0]}'><i class='icon-edit'></i></a></td>";
			echo "<td><a href='#' class='confirm-delete' data-id='{$linha[0]}'><i class='icon-trash'></i></a></td>";
			}
			echo "<td><a href='visualizarFunc.php?cdFunc={$linha[0]}'><i class='icon-search'></i></a></td>";
...
<div id="modal-from-dom" class="modal hide fade">
    ...
    <div class="modal-body">
	        <p>Deseja realmente excluir o Membro?</p>
        
        <p id="debug-url"></p>
    </div>
    <div class="modal-footer">
        <a href="deletarFunc.php?cdFunc=" class="btn danger">Sim</a>
