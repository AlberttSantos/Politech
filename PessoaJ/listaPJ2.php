...
$cont = 0;	
	// Listando os Cargos
	$resultado = mysql_query("select * from pessoaJ WHERE STATUS = 'A'"); // executa  query de consulta e armazena o resultado devolvido na variável $resultado
	...
	else
	{
		...
		echo "<th>Telefone</th>";
		echo "<th>Celular</th>";
		//Exibe as opções de editar e excluir apenas para nivel Diretor e ADM
		if ($nivel >= 4) {
		echo "<th>Editar</th>";
		echo "<th>Excluir</th>";
		}
		...
		while($linha = mysql_fetch_array($resultado))
		{
			...
			//Exibe as opções de editar e excluir apenas para nivel Diretor e ADM
			if ($nivel >= 4) {
			echo "<td><a href='editarPJ.php?cdPessoaJ={$linha[0]}'><i class='icon-edit'></i></a></td>";
			
			echo "<td><a href='#' class='confirm-delete' data-id='{$linha[0]}'><i class='icon-trash'></i></a></td>";
			}
			echo "<td><a href='visualizarPJ.php?cdPessoaJ={$linha[0]}'><i class='icon-search'></i></a></td>";
			echo "</tr>";
			$cont++;
		}
		echo "</table>";
...
<div id="modal-from-dom" class="modal hide fade">
    ...
    <div class="modal-footer">
        <a href="deletarPJ.php?cdPessoaJ=" class="btn danger">Sim</a>
